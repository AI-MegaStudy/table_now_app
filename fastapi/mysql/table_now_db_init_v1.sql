# MySQL 8.0 스키마 스크립트 v3 (소셜 로그인 및 비밀번호 변경 인증 반영)

-- > 기준: 최신 ERD\_02 + 예약(reserve) 구조 갱신본 + 소셜 로그인 + 비밀번호 변경 인증
...
-- 작성일: 2026-01-15
-- 작성자: 김택권
-- 설명: MySQL 8.0 데이터베이스 초기화 스크립트 - 모든 테이블을 Drop 후 Create하여 안정적으로 초기화
--       소셜 로그인(구글) 및 비밀번호 변경 인증 기능을 포함한 최신 스키마 반영
--
-- ============================================================
-- 수정 이력
-- ============================================================
-- 2026-01-15 김택권: 초기 생성
--   - ERD_02 기준으로 모든 테이블 생성 (customer, store, store_table, weather, reserve, menu, option, pay)
--   - customer 테이블에 소셜 로그인 관련 컬럼 추가 (provider, provider_subject)
